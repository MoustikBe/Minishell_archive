...
static char	*word_dup(char *str, int start, int finish)
{
	...
	{
		if(s_start == finish - 1 && str[s_start] == '"' || s_start == finish - 1 && str[s_start] == '\'')
			s_start++;
		else if(str[s_start] == '"' && inside == 0|| str[s_start] == '\'' && inside == 0)
		{
			inside = 1;
			s_start++;
		}
	...
	while(start < finish)
	{
		if(start == finish - 1 && str[start] == '"' || start == finish - 1 && str[start] == '\'')
			start++;
		else if(str[start] == '"' && inside == 0 || str[start] == '\'' && inside == 0)
		{
			inside = 1;
			start++;
		}
